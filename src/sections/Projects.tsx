...
function Projects() {
  ...
  const handleCarouselChange = (index: number) => {
    ...
    const projectDescription =
      projects[projectKey as keyof typeof projects].description;
    ...
    >
        ...
        <Carousel onSlideChange={handleCarouselChange} slide={false}>
          {entries.map(([key, p]) => {
            return (
              <div
                key={key}
                className="relative h-96 overflow-hidden rounded shadow-2xl border"
              >
                <h2 className="absolute z-10 m-3 text-2xl bg-black/50 text-white px-2 py-1 rounded">
                  {p.name}
                </h2>
                <Image
                  src={p.image}
      ...
      <div className="h-auto md:h-64 w-3/4 bg-slate-700 text-left dark:bg-slate-100 rounded-lg relative">
        ...
        <div className="ml-3 flex flex-wrap">
          {projects[
            Object.keys(projects)[tab] as keyof typeof projects
          ].techStack.map((item) => (
            <span
              key={item}
              className="bg-blue-400 m-1 p-2 rounded-md inline-block text-sm text-black"
            >
              {item}
            </span>
          ))}
        ...
        <div className="ml-auto p-3 lg:p-0 lg:ml-4 mt-4 flex flex-col space-y-2 lg:flex-row lg:items-end lg:space-y-0 lg:space-x-2">
          {projects[Object.keys(projects)[tab] as keyof typeof projects]
            .demo && (
            <Link
              href={
                projects[Object.keys(projects)[tab] as keyof typeof projects]
